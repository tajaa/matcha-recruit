...
/// shared canvas element id lets canvas_* update edit the wrong copy).
...
var isNull: Bool
static func from(_ any: Any) -> JSONValue // for schema `make`/`newItem` blobs
var anyValue: Any?
}
...
let config: [String: JSONValue] // Part A2 — required for set_theme preset
...
### B4. `Models/MerlinModels.swift`

Mirrors server/app/cappe/models/merlin.py.

struct CappeMerlinHistoryTurn: Codable { let role: String; let content: String; let ops_summary: String? }
...
func applyFieldPath(_ block: CappeBlock, path: String, value: JSONValue) -> CappeBlock?

/// One set_theme. `key` is a dot path: colors.brand, fonts.heading, type.*,
...
| canvas_* | y:cvNextY(els), w:8, h:2}; canvas_* update spreads patch (never id/kind — server already |
...
func record(_ s: EditorSnapshot, coalescing: Bool)
func checkpoint()
...
func create(siteId: String, _ body: CappePageCreate) async throws -> CappePage // POST /sites/{siteId}/pages
func update(siteId: String, pageId: String, _ body: CappePageUpdate) async throws -> CappePage // PUT
func delete(siteId: String, pageId: String) async throws // DELETE, 204
}

// Services/PreviewService.swift — routes/sites.py:211 preview_site_page
...
func conversation(_ id: String) async throws -> CappeMerlinConversationDetail // GET /merlin/conversations/{id}
...
func reportResults(messageId: String, _ results: [CappeMerlinOpResult]) async throws // PATCH /merlin/messages/{id}/results

/// SSE. POST /sites/{siteId}/merlin/agent.
func agent((siteId: String, _ body): CappeMerlinChatRequest,
onFrame: @escaping (CappeMerlinFrame) -> Void) async throws

/// SSE. POST /sites/{siteId}/merlin/setup/agent.
func setupAgent(siteId: String, _ body: CappeMerlinSetupRequest,
onFrame: @escaping (CappeMerlinFrame) -> Void) async throws
...
func updateUIView(_ v: WKWebView, context: Context) // reload only when html changes;
...
### C2. `PageEditorView.swift` and `PageEditorViewModel.swift`
...
Every control emits exactly one vm.setThemeKey(_: _) → applyThemeOp — one code path shared with Merlin.
...
### C5. `MerlinChatView.swift` and `MerlinChatViewModel.swift`
