...
# ── behavioral friction & retention risk: RETIRED ────────────────────────────
# Removed from evaluate_trends on 2026-06-08 (EB-broker feature, deemed low
# value) — see the note in broker_risk_alerts.py where the block used to sit.
# The eight tests that exercised the spike / cold-start / escalation / hot-
# location rules went with it; what is left is the one assertion that still
# has teeth: a "behavioral" block on the metrics must not resurrect the alert.
def test_behavioral_friction_is_retired_and_never_fires():
    behavioral = {
        "window_days": 90,
        "current_count": 12, "prior_count": 4, "delta_pct": 200.0,
        "attendance_count": 4, "insubordination_count": 2,
        "hot_location": {"name": "North Clinic", "count": 12},
    }
    # Numbers chosen to clear every retired threshold (delta 200% >= 50 * 2.0,
    # abs rise 8 >= 3), so a reinstated rule would fire critical here.
    m = _metrics(behavioral=behavioral)
    assert "behavioral_friction" not in _keys(evaluate_trends(m))
    # ...and nothing else fires off the behavioral block either.
    assert not any("behavioral" in str(c.get("metric_key")) for c in evaluate_trends(m))
