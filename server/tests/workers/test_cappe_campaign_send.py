...
# Deliverable addresses must not be RFC-2606 reserved (deliverable_recipients
# drops those); the fake email service below never sends anything.
SUBSCRIBERS = [
    {"id": "sub-ok", "email": "ok@subscriber-one.com", "name": "A", "unsubscribe_token": "t1"},
    {"id": "sub-bad", "email": "bad@subscriber-two.com", "name": "B", "unsubscribe_token": "t2"},
]
...
def test_failed_recipient_is_counted_and_logged(monkeypatch, caplog):
    ...
    assert svc.sent_to == ["ok@subscriber-one.com"]
    ...
    assert not any("bad@subscriber-two.com" in m for m in messages)
