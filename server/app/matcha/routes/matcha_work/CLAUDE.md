...
| `workspace.py` | Cross-project home surface: open-tasks/recent-activity feeds, per-user Gmail email agent, entitlements/usage. Email AI (summarize / draft reply / in-app triage) is one-shot Flash Lite in `services/matcha_work/email_ai_service.py`, gated Lite+ `email_ai`; reads and sends stay free. `POST /agent/email/snapshot` renders the caller's messages to `email-<message id>.md` task attachments (through `project_file_service.store_project_file_bytes`, the same upload policy as a person's upload) — the corpus an `email` kanban card hands the AutoPR lane. The Google OAuth callback is exported on its own ungated router at the unchanged `/matcha-work/agent/email/callback` path because Google's system-browser redirect carries signed state but no Matcha bearer token. The global (non-project) manual task board moved to `routes/dashboard/tasks.py` (2026-07-28) — see that package's CLAUDE.md — so this no longer holds an order-sensitive route pair | 15 + 1 OAuth callback |
...
| `connectors.py` | AI connectors (Claude / ChatGPT on the person's own plan): OAuth consent GET/POST (the SPA `/oauth/consent` page's API), connected-assistant list + disconnect, and `POST /projects/{p}/tasks/{t}/research-launch` (deep link / `claude` command for a research card). The MCP endpoint + OAuth protocol routes are NOT here — `main.py` mounts them from `routes/mcp_connector/`. See `docs/ops/MCP_CONNECTOR.md` | 5 |
