...
from app.matcha.models.scheduling.employee_schedule import (
    JobCreate, JobEmployeesReplace, JobUpdate,
)
...
_JOB_COLS = "id, company_id, location_id, name, color, notes, created_by, created_at, updated_at"
...
async def list_jobs(
    ...
    return {
        "jobs": [serialize_job(row, employees_by_job.get(str(row["id"]), [])) for row in rows]
    }
...
async def create_job(body: JobCreate, current_user=Depends(require_admin_or_client)):
        ...
        async with conn.transaction():
            row = await conn.fetchrow(
                ...
                INSERT INTO schedule_jobs
                    (company_id, location_id, name, color, notes, created_by)
                VALUES ($1, $2, $3, $4, $5, $6)
                RETURNING {_JOB_COLS}
                """,
    ...
    return serialize_job(row, [str(employee_id) for employee_id in employee_ids])
...
async def get_job(job_id: UUID, current_user=Depends(require_admin_or_client)):
    ...
    return serialize_job(row, employee_ids)
...
async def update_job(
    ...
    return serialize_job(row, [str(r["employee_id"]) for r in employee_rows])
...
async def replace_job_employees(
    ...
    async with get_connection() as conn:
        ...
        async with conn.transaction():
            await conn.execute(
                "DELETE FROM schedule_job_employees WHERE job_id = $1 AND company_id = $2",
                job_id, company_id,
            )
            for employee_id in employee_ids:
                await conn.execute(
