...
| `onboarding/sc.py` | `/sc-onboarding` | Matcha S&C first-account setup wizard backend — `status` + `csv/{kind}` (parse-only, writes nothing) + atomic `complete` (company size/NAICS, locations + roster CSV, schedule jobs, tenant credential types). Only reachable for a custom product whose `product_definitions.onboarding_kind='sc'` (migration `sconboard01`); completion stamps `companies.sc_onboarding_completed_at`. CSV parsing is server-side (`services/employees/roster_csv.py`, shared with `employees/bulk_upload.py`) so the wizard cannot accept a file `complete` would refuse. Mounted with `require_all_features("employees", "employee_schedule", "credential_templates")` — `is_tenant_activated` alone admits a `free`-priced product whose flags an admin later turned off |
...
| `dashboard/` | `/dashboard` | Cross-feature dashboard aggregation — **package** (split 2026-07-26, 20 routes as of the 2026-07-28 task-board move; see `dashboard/CLAUDE.md`). Ungated (no `require_feature` at the mount) — the global manual-task board (`tasks.py`) lives here specifically so it's visible to every tenant, not just `matcha_work` companies |
...
| `matcha_work/` | (multiple: `/matcha-work`, `/matcha-work/public`, `/matcha-work/presence`; plus the unprefixed `tutor_sessions_router` sibling it exports but does not include) | Matcha-work projects/threads/tasks/recruiting/AI turns — **package** (split 2026-07-03, 200 routes as of the 2026-07-29 huume record-close route; see `matcha_work/CLAUDE.md`) |
