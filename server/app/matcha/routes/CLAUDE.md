...
| `onboarding/sc.py` | `/sc-onboarding` | Matcha S&C first-account setup wizard backend — `status` + atomic `complete` (company size/NAICS, locations + roster CSV, schedule jobs, tenant credential types). Only reachable for a custom product whose `product_definitions.onboarding_kind='sc'` (migration `sconboard01`); completion stamps `companies.sc_onboarding_completed_at`. Mounted with `require_all_features("employees", "employee_schedule", "credential_templates")` — `is_tenant_activated` alone admits a `free`-priced product whose flags an admin later turned off |
