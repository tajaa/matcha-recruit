...
async def generate_suggested_guidance(
    ...
    try:
        ...
        try:
            # Single statement: the snapshot in case_row predates two Gemini
            # calls, so a read-modify-write here loses concurrent intake edits.
            async with get_connection() as cache_conn:
                await cache_conn.execute(
                    "UPDATE er_cases SET intake_context = jsonb_set("
                    "COALESCE(intake_context, '{}'::jsonb), '{last_guidance}', $1::jsonb, true) WHERE id = $2",
