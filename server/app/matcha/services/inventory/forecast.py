...
def build_reorder_plan(lines: Iterable[Mapping], *, today: date) -> dict:
    ...
    for raw in lines:
        ...
        if extended_cost is None:
            uncosted_count += 1
        plan_lines.append({
            "item_id": line.get("item_id"), "name": line.get("name"), "unit": line.get("unit"),
            "suggested_quantity": suggested, "average_daily_demand": line.get("average_daily_demand"),
            "lead_demand": line.get("lead_demand"), "runout_date": line.get("runout_date"),
            "order_by_date": order_by, "days_until_order_by": days_until, "urgency": urgency,
