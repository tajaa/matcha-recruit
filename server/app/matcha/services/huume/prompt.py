    ...
    if isinstance(action, dict) and action.get("status") == "proposed":
        if action.get("type") == "send_offer":
            ...
            if isinstance(operation_count, int) and operation_count > 1:
                detail = describe_operations(operation_count, action.get("operation_summary"))
            else:
            ...
            if isinstance(review, dict):
                ...
                if summary["unfilled"]:
                    ...
                    # The count alone is not relayable, and a model with only a
                    # count reconstructs a reason from the roster instead.
                    for entry in summary["unfilled_reasons"][:3]:
                        lines.append(
                            f"    Blocker: {entry['reason']} ({entry['seats']} seat(s))"
                        )
