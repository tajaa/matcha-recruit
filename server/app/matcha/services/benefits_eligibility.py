...
def _kit_html(company_name: str, detail: dict) -> str:
    ...
    for d in detail.get("dimensions", []):
        if d["risk_band"] == "stable":
            continue
        triggers = "".join(f"<li>{t}</li>" for t in d.get("triggers", []))
        rows += f"""
        <div class="dim">
          <h3>{d['dimension_type'].title()}: {d['dimension_value'] or '(company-wide)'}
              <span class="band {d['risk_band']}">{d['risk_band'].upper()}</span></h3>
    ...
    return f"""<!DOCTYPE html><html><head><style>
      ...
      <p class="sub">{company_name} — {policy_line} · Renewal exposure: {detail.get('risk_band','').upper()}</p>
      {rows or '<p>No elevated locations or departments.</p>'}
      <div class="rec"><strong>Recommendation</strong><br/>{detail.get('recommendation','')}</div>
...
async def render_stabilization_kit_pdf(company_name: str, detail: dict) -> bytes:
    ...
    def _render() -> bytes:
        ...
        return HTML(string=_kit_html(company_name, detail)).write_pdf()
