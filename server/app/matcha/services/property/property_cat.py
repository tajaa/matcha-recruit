...
# `geocode` / `geocode_fips` MOVED to `core/services/geo.py` (2026-07-28) so
# Cappe's Discover directory can geocode without a `cappe -> matcha` import —
# that edge is 0 today and the root CLAUDE.md keeps it that way (Tell-Us holds
# the only documented exception). Re-exported here unchanged and NOT deprecated:
# `tellus/services/geo.py` imports `geocode` through this module and CLAUDE.md
# requires its signature stay stable, so this name must keep resolving.
# `_get_json` above stays local — the FEMA/USGS/USFS peril calls below use it.
from ....core.services.geo import geocode, geocode_fips  # noqa: E402,F401
