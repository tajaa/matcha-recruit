...
async def _node_context(company_id: UUID) -> str:
    from app.matcha.services.matcha_work.matcha_work_node import build_node_context
    return await build_node_context(company_id)


async def _benefits_context(company_id: UUID) -> str:
    from app.matcha.services.matcha_work.matcha_work_mode_contexts import build_benefits_context
    return await build_benefits_context(company_id)


async def _legal_context(company_id: UUID) -> str:
    from app.matcha.services.matcha_work.matcha_work_mode_contexts import build_legal_context
    return await build_legal_context(company_id)


async def _risk_context(company_id: UUID) -> str:
    from app.matcha.services.matcha_work.matcha_work_mode_contexts import build_risk_context
    return await build_risk_context(company_id)


async def _training_context(company_id: UUID) -> str:
    from app.matcha.services.matcha_work.matcha_work_mode_contexts import build_training_context
    return await build_training_context(company_id)


async def _hr_pilot_context(company_id: UUID) -> str:
    from app.matcha.services.matcha_work.matcha_work_mode_contexts import build_hr_pilot_context
...
THREAD_MODES: tuple[ThreadMode, ...] = (
    ...
    ThreadMode(
        ...
        label="Benefits",
        status_loading="Loading benefits plans, enrollment & eligibility data...",
