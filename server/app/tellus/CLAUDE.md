...
- **`accounts.py`** — list/search/detail, suspend/unsuspend (`tellus_accounts.status`, CHECK'd to `active|suspended` since `tellus_app_08`, previously never written after INSERT), force sign-out (`tokens_valid_after = NOW()`, same write self-logout uses), verify-email, password-reset link mint (Tell-Us had no reset flow before — `tellus_password_reset_tokens` + public `POST /auth/reset-password` consume in `routes/auth.py`, 1h expiry, single-use, revokes all sessions), manual points adjust, and grant/revoke of admin-gifted consumer paid tiers (`consumer_tier` with optional expiry; audit actions `account.tier_grant`/`account.tier_revoke`).
...
- **Every `owner_account_id` writer must also insert a `tellus_brand_members` owner row.** Call sites patched for this in the same PR: brand signup (`routes/auth.py`), `routes/admin/brands.py:assign_owner`, `routes/admin/claims.py:approve_claim`, plus a one-time backfill in `tellus_app_12`. The two in-code owner-fallbacks this note used to warn about (`board_service.py:resolve_moderated_brand` / `routes/board.py:get_board` falling back to `brand.owner_account_id == account.id`) no longer exist — access is now resolved entirely through `access_service.py`'s `find_brand_access`/`ROLE_CAPABILITIES` (owner/admin/location_manager/staff), and prod has zero owners without an active membership row (verified 2026-08-16). Keep the INSERT-a-membership-row invariant regardless — it's what `find_brand_access` depends on now.
