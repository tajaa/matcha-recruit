...
async def _brand_stats(conn, brand_account_id: UUID) -> BrandStats:
    ...
    return BrandStats(
        ...
        avg_hours_to_pay=float(row["avg_hours_to_pay"]) if row["avg_hours_to_pay"] is not None else None,
    )
...
async def _offer_detail(conn, offer_id: UUID, account_id: UUID) -> OfferDetail:
    ...
    revision_rows = await conn.fetch(
        "SELECT * FROM cappe_collab_offer_revisions WHERE offer_id = $1 ORDER BY revision_no", offer_id
    )
    message_rows = await conn.fetch(
        "SELECT * FROM cappe_collab_messages WHERE offer_id = $1 ORDER BY created_at", offer_id
    )
    deliverable_rows = await conn.fetch(
        "SELECT * FROM cappe_collab_deliverables WHERE offer_id = $1 ORDER BY idx", offer_id
    )
    ...
    revisions = [
        OfferRevisionOut(
            id=r["id"], revision_no=r["revision_no"], proposed_by=r["proposed_by"],
            terms=loads(r["terms"]), message=r["message"], created_at=r["created_at"],
        )
    ...
    return OfferDetail(
        ...
        currency=offer_row["currency"], campaign_id=offer_row["campaign_id"],
        brand_name=offer_row["brand_name"], creator_handle=offer_row["creator_handle"],
        creator_display_name=offer_row["creator_display_name"],
        creator_avatar_url=offer_row["creator_avatar_url"],
        last_action_at=offer_row["last_action_at"], created_at=offer_row["created_at"],
        side=side, accepted_revision_id=offer_row["accepted_revision_id"],
        declined_reason=offer_row["declined_reason"], cancelled_by=offer_row["cancelled_by"],
...
@router.post("/collab/campaigns", status_code=status.HTTP_201_CREATED, response_model=Campaign)
async def create_campaign(body: CampaignUpsert, account: CappeAccount = Depends(require_cappe_account)):
    _require_business(account)
    async with get_connection() as conn:
        row = await conn.fetchrow(
            ...
            account.id, body.title, body.description, body.budget_min_cents,
            body.budget_max_cents, body.deliverable_notes,
        )
        return Campaign(**dict(row))


@router.patch("/collab/campaigns/{campaign_id}", response_model=Campaign)
async def update_campaign(campaign_id: UUID, body: CampaignPatch, account: CappeAccount = Depends(require_cappe_account)):
    _require_business(account)
    fields = body.model_fields_set
    sets, args = [], []
    for col in ("title", "description", "budget_min_cents", "budget_max_cents", "deliverable_notes", "status"):
        if col in fields:
...
@router.post("/collab/offers", status_code=status.HTTP_201_CREATED, response_model=OfferDetail)
async def create_offer(
    body: OfferCreate, background: BackgroundTasks, account: CappeAccount = Depends(require_cappe_account)
):
    _require_business(account)
    ...
    async with get_connection() as conn:
        ...
        if target is None or target["status"] != "published" or not target["open_to_offers"]:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This creator is not accepting offers")
        ...
        background.add_task(
            send_cappe_offer_received_email, creator_contact["email"], creator_contact["name"],
            account.name or account.email, body.title, dashboard_url(f"/creator/deals/{offer_id}"),
        )
    ...
    async with get_connection() as conn:
        if side == "brand":
            ...
            rows = await conn.fetch(
                ...
                account.id, statuses, limit, offset,
            )
        else:
        ...
        if active_ids:
            ...
            refreshed = {
                r["id"]: r for r in await conn.fetch(
                    "SELECT id, status, total_cents, last_action_at FROM cappe_collab_offers WHERE id = ANY($1)",
                    active_ids,
                )
            }
            for d in row_dicts:
                if d["id"] in refreshed:
                    r2 = refreshed[d["id"]]
                    d["status"], d["total_cents"], d["last_action_at"] = r2["status"], r2["total_cents"], r2["last_action_at"]
...
async def get_offer(offer_id: UUID, account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        return await _offer_detail(conn, offer_id, account.id)
...
async def counter_offer(
    offer_id: UUID, body: OfferCounter, background: BackgroundTasks,
    account: CappeAccount = Depends(require_cappe_account),
):
    await _write_rate_limit(account.id)
    async with get_connection() as conn:
        ...
        async with conn.transaction():
            ...
            rev_id = await conn.fetchval(
                ...
                offer_id, rev_no, side, json.dumps(body.terms.model_dump(mode="json")), body.message,
            )
            await conn.execute(
            ...
            if body.message:
                await conn.execute(
                    ...
                    "VALUES ($1,$2,$3,$4,$5)",
                    offer_id, side, account.id, body.message, rev_id,
                )

        counterpart_email, counterpart_name = await _resolve_contact(conn, offer_row, side)
        background.add_task(
            send_cappe_offer_counter_email, counterpart_email, counterpart_name,
            account.name or account.email, offer_row["title"],
            dashboard_url(f"/{'creator/deals' if side == 'brand' else 'collabs'}/{offer_id}"),
        )
        return await _offer_detail(conn, offer_id, account.id)


async def _resolve_contact(conn, offer_row, side: str) -> tuple[str, Optional[str]]:
    """(email, name) of the account OPPOSITE `side`."""
    if side == "brand":
        row = await conn.fetchrow(
            "SELECT ca.email, ca.name FROM cappe_creator_profiles p JOIN cappe_accounts ca ON ca.id = p.account_id "
            "WHERE p.id = $1",
...
async def accept_offer_route(offer_id: UUID, background: BackgroundTasks, account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        async with conn.transaction():
        ...
        funding_due = any(p.trigger == "on_accept" and p.status == "due" for p in detail.payments)
        ...
        background.add_task(send_cappe_offer_accepted_email, brand_email, brand_name, offer_row["title"],
                            dashboard_url(f"/collabs/{offer_id}"), funding_due=funding_due)
        background.add_task(send_cappe_offer_accepted_email, creator_email, creator_name, offer_row["title"],
                            dashboard_url(f"/creator/deals/{offer_id}"), funding_due=False)
        return detail


@router.post("/collab/offers/{offer_id}/decline", response_model=OfferDetail)
async def decline_offer(offer_id: UUID, body: OfferDecline, background: BackgroundTasks,
                        account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "creator":
...
async def withdraw_offer(offer_id: UUID, background: BackgroundTasks, account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "brand":
...
async def cancel_offer_route(offer_id: UUID, body: OfferCancel, background: BackgroundTasks,
                             account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        async with conn.transaction():
        ...
        link = dashboard_url(f"/{'creator/deals' if side == 'brand' else 'collabs'}/{offer_id}")
        background.add_task(send_cappe_offer_closed_email, email, name, offer_row["title"], "cancelled",
                            body.reason, link)
        return await _offer_detail(conn, offer_id, account.id)


@router.post("/collab/offers/{offer_id}/messages", status_code=status.HTTP_201_CREATED, response_model=OfferMessageOut)
async def send_message(offer_id: UUID, body: OfferMessageCreate, background: BackgroundTasks,
                       account: CappeAccount = Depends(require_cappe_account)):
    await _write_rate_limit(account.id)
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        ...
        row = await conn.fetchrow(
            ...
            offer_id, side, account.id, body.body,
        )
        ...
        if await _recipient_send_ok(email):
            link = dashboard_url(f"/{'creator/deals' if side == 'brand' else 'collabs'}/{offer_id}")
            background.add_task(send_cappe_collab_message_email, email, name,
                                account.name or account.email, offer_row["title"], body.body, link)
        return OfferMessageOut(**dict(row))


# ── Deliverables ─────────────────────────────────────────────────────────────

@router.post("/collab/offers/{offer_id}/deliverables/{deliverable_id}/submit", response_model=DeliverableOut)
async def submit_deliverable(offer_id: UUID, deliverable_id: UUID, body: DeliverableSubmit,
                             background: BackgroundTasks, account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "creator":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the creator can submit deliverables")
        if offer_row["status"] != "active":
        ...
        row = await conn.fetchrow(
            ...
            deliverable_id, body.submission_url, body.submission_note, body.proof_media_url, offer_id,
        )
        if row is None:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Already submitted or approved")
        await svc.touch(conn, offer_id)
        email, name = await _resolve_contact(conn, offer_row, "creator")
        background.add_task(send_cappe_deliverable_submitted_email, email, name, offer_row["title"],
                            f"{row['type']} #{row['idx'] + 1}", dashboard_url(f"/collabs/{offer_id}"))
        return DeliverableOut(**dict(row))


@router.post("/collab/offers/{offer_id}/deliverables/{deliverable_id}/approve", response_model=OfferDetail)
async def approve_deliverable(offer_id: UUID, deliverable_id: UUID, background: BackgroundTasks,
                              account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "brand":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the brand can approve deliverables")
        ...
        if due_ids:
            pay_rows = await conn.fetch(
                "SELECT id, label, amount_cents FROM cappe_collab_payments WHERE id = ANY($1)", due_ids
            )
            for p in pay_rows:
...
@router.post("/collab/offers/{offer_id}/deliverables/{deliverable_id}/request-revision", response_model=DeliverableOut)
async def request_deliverable_revision(offer_id: UUID, deliverable_id: UUID, body: DeliverableRevision,
                                       background: BackgroundTasks, account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "brand":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the brand can request revisions")

        deliverable = await conn.fetchrow(
            "SELECT * FROM cappe_collab_deliverables WHERE id=$1 AND offer_id=$2", deliverable_id, offer_id
        )
        ...
        row = await conn.fetchrow(
            ...
            "revision_count=revision_count+1, updated_at=NOW() WHERE id=$1 RETURNING *",
            deliverable_id, body.review_note,
        )
        email, name = await _resolve_contact(conn, offer_row, "brand")
        background.add_task(send_cappe_deliverable_decision_email, email, name, offer_row["title"],
                            f"{row['type']} #{row['idx'] + 1}", False, body.review_note,
                            dashboard_url(f"/creator/deals/{offer_id}"))
        return DeliverableOut(**dict(row))
...
async def checkout_payment(offer_id: UUID, payment_id: UUID, account: CappeAccount = Depends(require_cappe_account)):
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "brand":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the brand can pay")
        payment = await conn.fetchrow(
            "SELECT * FROM cappe_collab_payments WHERE id=$1 AND offer_id=$2", payment_id, offer_id
        )
        ...
        session = await get_cappe_stripe().create_checkout_session(
            ...
            line_items=[{
                "price_data": {
                    ...
                    "product_data": {"name": f"{offer_row['title']} — {payment['label']}"},
                },
                "quantity": 1,
            }],
            ...
            metadata={"collab_payment_id": str(payment_id), "offer_id": str(offer_id),
    ...
    async with get_connection() as conn:
        await conn.execute(
            ...
            "fee_bps_snapshot=$3, fee_cents=$4, updated_at=NOW() WHERE id=$1",
...
async def nudge_payment(offer_id: UUID, payment_id: UUID, background: BackgroundTasks,
                        account: CappeAccount = Depends(require_cappe_account)):
    ...
    async with get_connection() as conn:
        offer_row, side = await svc.get_offer_side(conn, offer_id, account.id)
        if side != "creator":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the creator can nudge")
        payment = await conn.fetchrow(
            "SELECT * FROM cappe_collab_payments WHERE id=$1 AND offer_id=$2", payment_id, offer_id
        )
        if payment is None or payment["status"] not in ("due", "processing"):
        ...
        email, name = await _resolve_contact(conn, offer_row, "creator")
        background.add_task(send_cappe_collab_payment_nudge_email, email, name, offer_row["title"],
                            payment["label"], payment["amount_cents"], dashboard_url(f"/collabs/{offer_id}"))
    return {"ok": True}
