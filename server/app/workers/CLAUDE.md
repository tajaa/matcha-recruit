...
- `schedule_auto_generation` prepares the upcoming Sunday-starting schedule for every active location whose company has merged `employee_schedule` + `huume` + `matcha_work`. It runs through the worker-ready dispatcher but is idempotent per location/week (`schedule_generation_runs.origin='automatic'` partial unique index), so repeated 15-minute dispatches are cheap after a proposal exists. It uses the shared deterministic planner via the pool-free `connection_or_direct` path, writes a review proposal only, and never creates or publishes shifts. A manager cancellation is terminal for that automatic location/week; a stale/failed run may be rebuilt. The schedule assistant adopts the proposal only when an authorized manager opens it.
