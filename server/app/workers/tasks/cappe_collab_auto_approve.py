"""Scheduled reconciliation for overdue Cappe collab deliverables."""
...
async def _run() -> dict:
    ...
    try:
        setting = await scheduler_settings_row(conn, "cappe_collab_auto_approve")
        if not setting or not setting["enabled"]:
            return {"skipped": True}
        cap = setting["max_per_cycle"] or 50
        offers = await conn.fetch(
            "SELECT o.*, ba.name AS brand_name FROM cappe_collab_offers o "
            "JOIN cappe_accounts ba ON ba.id = o.brand_account_id "
            "WHERE o.status = 'active' ORDER BY o.last_action_at ASC LIMIT $1",
            cap,
        )
        approved = 0
        for offer in offers:
