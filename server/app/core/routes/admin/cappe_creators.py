...
_MARKETPLACE_SETTINGS_DEFAULTS = {"collab_fee_bps": 1500, "min_offer_cents": 5000}
...
async def admin_list_creators(status_filter: Optional[str] = None):
    ...
    for r in rows:
        ...
        d["reaudit_due"] = bool(
            r["reach_audited_at"] is not None
            and r["reach_audited_at"] < datetime.now(timezone.utc) - timedelta(days=90)
        )
        out.append(d)
    return out
...
async def admin_get_marketplace_settings():
    ...
    return {
        "collab_fee_bps": values.get("collab_fee_bps", {}).get("bps", _MARKETPLACE_SETTINGS_DEFAULTS["collab_fee_bps"]),
        "min_offer_cents": values.get("min_offer_cents", {}).get("cents", _MARKETPLACE_SETTINGS_DEFAULTS["min_offer_cents"]),
        "auto_approve_days": values.get("auto_approve_days", {}).get("days", 14),
    }
    ...
    async with get_connection() as conn:
        ...
        brand_rows = await conn.fetch(
            """SELECT ba.id AS brand_account_id, ba.name AS brand_name, ba.email AS brand_email,
                      COUNT(*) FILTER (WHERE o.status = 'sent') AS offers_sent,
