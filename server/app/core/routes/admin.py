...
KNOWN_FEATURES = {
    ...
    "handbook_pilot", "analysis_pilot", "hr_pilot",
}
...
async def eval_baseline_checklist():
    ...
    """
    from ..services.compliance_evals.baseline import baseline_scorecard
...
# Moved to the service layer (scope_registry/jurisdiction_chain.py) so the
# scope-registry resolver can share it — services must not import from routes.
# Kept under its old private name for this module's callsites.
...
async def deal_flow_broker_defaults():
    ...
    from ..services.deal_broker import DEFAULT_BROKER_BLOCKS, DEFAULT_MARGIN_TIERS
...
async def deal_flow_broker_preview(inp: BrokerInputs):
    ...
    from ..services.deal_broker import compute_broker_quote
    from ..services.deal_broker_template import render_broker_proposal_html
