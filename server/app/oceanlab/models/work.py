...
class RecordingWork(Base):
    ...
    recording_id: Mapped[uuid.UUID] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_recordings.id", ondelete="CASCADE"), primary_key=True
    )
    work_id: Mapped[uuid.UUID] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_works.id", ondelete="CASCADE"), primary_key=True
    )
...
class WorkWriter(Base, TimestampMixin):
    ...
    __table_args__ = (
        sa.UniqueConstraint("work_id", "contributor_id", "role", name="uq_oceanlab_work_writers_work_contributor_role"),
    )

    id: Mapped[uuid.UUID] = mapped_column(sa.Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    work_id: Mapped[uuid.UUID] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_works.id", ondelete="CASCADE"), nullable=False
    )
