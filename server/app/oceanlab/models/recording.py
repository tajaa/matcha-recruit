...
class Credit(Base, TimestampMixin):
    ...
    recording_id: Mapped[uuid.UUID] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_recordings.id", ondelete="CASCADE"), nullable=False
    )
...
class MasterSplit(Base, TimestampMixin):
    ...
    __table_args__ = (
        sa.UniqueConstraint("recording_id", "contributor_id", name="uq_oceanlab_master_splits_recording_contributor"),
    )
