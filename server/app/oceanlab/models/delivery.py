...
from app.oceanlab.models.enums import DeliveryStatus, DeliveryTarget


class Delivery(Base, TimestampMixin):
    __tablename__ = "oceanlab_deliveries"
    ...
    package_file_id: Mapped[uuid.UUID | None] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_files.id"), nullable=True
    )
...
class DeliveryItem(Base, TimestampMixin):
    __tablename__ = "oceanlab_delivery_items"
    __table_args__ = (
        sa.UniqueConstraint("delivery_id", "track_id", name="uq_oceanlab_delivery_items_delivery_track"),
    )

    id: Mapped[uuid.UUID] = mapped_column(sa.Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    delivery_id: Mapped[uuid.UUID] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_deliveries.id", ondelete="CASCADE"), nullable=False
    )
    track_id: Mapped[uuid.UUID] = mapped_column(
        sa.Uuid(as_uuid=True), sa.ForeignKey("oceanlab_tracks.id", ondelete="CASCADE"), nullable=False
    )
    status: Mapped[DeliveryStatus] = mapped_column(
        sa.Enum(DeliveryStatus, native_enum=False, create_constraint=True, name="status"),
        nullable=False,
