...
`werk → matcha.services` is **allowed and intentional**: 9 files / 59 import statements (verified 2026-08-03 by direct grep — `grep -rnE "^\s*(from|import)\s+(app\.|\.\.\.)matcha" server/app/werk/`), reaching ten things — `notification_service` (the bulk of them), `matcha_work.project_file_service`/`mentions`, `billing.entitlements_service`, `billing.token_budget_service.FREE_TOKEN_GRANT`, `matcha.dependencies` (`resolve_accessible_company_scope`, `require_admin_or_client`), `ems.*` (`ask`/`intent`/`protocols`/`event_intake`/`urgent_notify`/`channel_grounding` — channels_ws.py's Huume/EMS message dispatch, `channel_grounding` new for channel-Huume's schedule/incidents/inventory/HR-ops grounding), `scheduling.schedule_chat`/`schedule_chat_rules`, `ir.report_links`, `inventory.*` (new — `movements`/`orders`/`pills`/`extraction`/`reorder`/`rules`, channels_ws.py's `_bg_inventory_request`/`_bg_inventory_reply`), and `_shared.uploads.read_upload_capped` (`inbox.py`'s capped-read upload gate). All lazy in-function imports **except one**: `routes/channels.py:18` imports `matcha.dependencies` at module level (FastAPI route dependencies must resolve at decoration time) — don't "fix" that into a lazy import.
