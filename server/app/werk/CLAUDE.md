...
`werk → matcha.services` is **allowed and intentional**: 9 files / 25 import statements, reaching five things — `notification_service`, `matcha_work.project_file_service`/`mentions`, `billing.entitlements_service`, `billing.token_budget_service.FREE_TOKEN_GRANT`, and `matcha.dependencies` (`resolve_accessible_company_scope`, `require_admin_or_client`). All lazy in-function imports **except one**: `routes/channels.py:18` imports `matcha.dependencies` at module level (FastAPI route dependencies must resolve at decoration time) — don't "fix" that into a lazy import.
