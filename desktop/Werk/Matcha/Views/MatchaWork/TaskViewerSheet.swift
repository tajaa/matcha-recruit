...
struct TaskViewerSheet: View {
    ...
    @State private var viewMode: ViewerMode = .list
    ...
    @ViewBuilder
    private func modeButton(_ mode: ViewerMode, icon: String) -> some View {
    ...
    private var graphModel: (nodes: [GraphNode], lanes: [GraphLane]) {
        let sorted = history
    ...
    @ViewBuilder
    private func laneAvatar(_ lane: GraphLane, size: CGFloat) -> some View {
        if lane.isSystem {
            Circle()
                .fill(Color.zinc800)
    ...
    private func graphLegend(_ lanes: [GraphLane]) -> some View {
        HStack(spacing: 12) {
    ...
    private func graphRoundBand(round: Int) -> some View {
        HStack(spacing: 6) {
            Text("ROUND \(round)")
...
private struct EventRow: View {
    ...
    fileprivate static func tint(for event: String) -> Color {
        switch event {
...
/// activity graph.
private enum ViewerMode { case list, graph }
...
private struct GraphLane: Identifiable {
