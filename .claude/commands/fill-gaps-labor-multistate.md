Research and write labor compliance data for jurisdictions across MULTIPLE states in one run. Covers 12 categories: minimum_wage, overtime, sick_leave, leave, meal_breaks, final_pay, pay_frequency, scheduling_reporting, workers_comp, workplace_safety, anti_discrimination, minor_work_permit.

Parse `$ARGUMENTS` for:
- `--states <ST1,ST2,...>` (required) — comma-separated two-letter state codes (e.g. `CA,TX,FL,NY`)
...
## Step 2–5: Loop over each state

**For EACH state in the `--states` list, execute Steps 2 through 5 below before moving to the next state.** Process states sequentially — finish one state completely before starting the next.
...
If `--list` was specified, report gaps for ALL states and stop.

### Step 3: Research via WebSearch (per state)

Research each missing category for the current state. **Labor laws vary significantly by state — most states have their own versions that differ from federal FLSA.**
...
5. For cities with their own ordinances (LA, SF, NYC, Seattle, Chicago, etc.), capture city-specific rules too
...
If `--dry-run`, write markdown to `server/scripts/<state>_labor_gaps.md` for each state and stop (do not write to DB).

### Step 4: Write to database (per state)
...
### Step 5: Per-state report

After each state completes, print:
1. State name and code
2. Per category: keys filled vs skipped (federal-only)
3. Total jurisdiction_requirements created/updated for this state

Then **move to the next state** in the list.

## Step 6: Final summary

After ALL states are done:
1. Grand total across all states: rows inserted/updated
2. Per-state breakdown (one line each)
