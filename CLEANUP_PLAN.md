...
- **J6 mega-service sub-packaging** `core/services/compliance_service.py` (10,703 lines, 143 defs) and `handbook_service.py` (5,147; one ~2,900-line class) → sub-packages, matching the existing precedents. No behavior change. `handbook_service/` **DONE** (commit `77ee996`). `compliance_service/` split is tracked module-by-module in **[COMPLIANCE_SERVICE_SPLIT.md](./COMPLIANCE_SERVICE_SPLIT.md)** (the last structural split in the plan) — package keeps the `compliance_service` name so all 42 importers are untouched.
