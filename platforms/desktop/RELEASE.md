...
The end-to-end pipeline is wrapped in `desktop/Espresso/release.sh`. For day-to-day dev builds use `desktop/Espresso/run.sh` instead — it builds and launches the unsigned debug app without touching notarization.
...
Already wired into `desktop/Espresso/Matcha.xcodeproj`:
...
The app's sandbox entitlements live in `desktop/Espresso/Espresso/Matcha.entitlements` and are compatible with Developer ID distribution: app sandbox on, network client on, user-selected files read-only.

---

## Cutting a release

From `desktop/Espresso/`:
...
All artifacts go to `desktop/Espresso/build/release/`:
...
| Path | Purpose |
| --- | --- |
| `desktop/Espresso/run.sh` | Dev build + launch (unsigned debug) |
| `desktop/Espresso/release.sh` | Archive + sign + notarize + staple + zip/dmg |
| `desktop/Espresso/Matcha.xcodeproj` | Project with team + bundle ID wired in |
| `desktop/Espresso/Espresso/Matcha.entitlements` | Sandbox + network-client entitlements |
