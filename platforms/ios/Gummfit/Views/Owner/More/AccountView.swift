...
struct AccountView: View {
    ...
    var body: some View {
        List {
            ...
            Section("Plan") {
                if let subscription {
                    Text(subscription.plan_name ?? subscription.plan_code ?? "Unknown plan").gummfitSectionTitle()
                    GummfitStatusPill(status: subscription.status ?? "unknown")
                    if let end = subscription.current_period_end { Text("Renews \(end)").gummfitMuted() }
                    if !subscription.addons.isEmpty { ForEach(subscription.addons) { Text("\($0.name) × \($0.quantity)") } }
                }
