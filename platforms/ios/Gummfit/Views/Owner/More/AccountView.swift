...
private struct OfferComposerSheet: View {
    let reload: () async -> Void; @Environment(\.dismiss) private var dismiss; @State private var creatorId = ""; @State private var title = ""; @State private var campaignId = ""; @State private var cents = ""; @State private var type = "post"; @State private var platform = "instagram"; @State private var message = ""; @State private var error: String?
    var body: some View { NavigationStack { Form { ErrorBanner(message: error); TextField("Creator profile ID", text: $creatorId); TextField("Offer title", text: $title); TextField("Campaign ID (optional)", text: $campaignId); TextField("Compensation (cents)", text: $cents).keyboardType(.numberPad); TextField("Deliverable type", text: $type); TextField("Platform", text: $platform); TextField("Message", text: $message, axis: .vertical) }.navigationTitle("New offer").toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Send") { Task { await save() } }.disabled(creatorId.isEmpty || title.isEmpty || Int(cents) == nil) } } } }
