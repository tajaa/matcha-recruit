...
enum FlyerRenderer {
    ...
    /// Draws a raster layer upright in the renderer's CGContext coordinate
    /// space. The renderer consumes `cgImage` pixels directly, so UIKit's
    /// image orientation metadata is ignored and the context's Y direction
    /// must be normalized explicitly for every raster source.
