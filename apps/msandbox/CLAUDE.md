...
- **Compose is always invoked with `--project-directory <repo root>`.** The
  compose files sit here, but their `context: .` / `.env` / workspace default
  mean the repo root. `bin/agent-sandbox.sh` (`COMPOSE_PROJECT_DIR`),
  `cli/docker_runtime.py`, `cli/autopr_control.py`, `self-audit/audit.sh` all
  pass it; a new call site that forgets it silently builds from `sandbox/`.
- **`apps/` is in every model-patch denylist** (`harness/run-codex-sandboxed.sh`
  `PATH_DENY_RE`, `harness/publish.sh`, `error-autofix/publish.sh`,
  `cli/validation.py`). This directory is the control plane; a patch that
  edits it must be refused, not reviewed.
- **The self-audit lane's allowed-edit set** is the regex in
  `self-audit/publish.sh` — extend it when adding a subdirectory here.
