    ...
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "case \"$dispatch_command\" in\n"
                "  ''|--menu|--dashboard)\n"
                "    attach_mode=${MSANDBOX_START_ATTACH:-menu}\n"
                "    case \"$dispatch_command\" in\n"
                "      --menu) attach_mode=menu; shift ;;\n"
                "      --dashboard) attach_mode=dashboard; shift ;;\n"
                "    esac\n"
                ...
                "      # Only `--dashboard` attaches the observer first; Ctrl-b d then opens\n"
                "      # the session manager. Scripts, pipes, and nested tmux clients skip it.\n"
