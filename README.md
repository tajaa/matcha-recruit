...
| **Werk** | macOS desktop client for matcha-work | `desktop/Werk/` (SwiftUI) | same matcha-work backend | App Store bundle `com.ahnimal.matcha` |
...
| **MatchaTutor** | iOS language tutor (dormant) | `ios/` (SwiftUI) | matcha-work tutor endpoints | — |
...
desktop/    Werk macOS app (SwiftUI)
ios/        MatchaTutor (SwiftUI, dormant)
...
```bash
./build-and-push.sh --frontend-only   # build + push image(s) to ECR
./update-ec2.sh --matcha              # blue-green swap on the app EC2
```
