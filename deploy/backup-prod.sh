#!/bin/bash
# Logical pg_dump of the self-hosted prod Postgres → S3, run ON the app EC2.
#
...
# container" on every run since the RDS cutover). RDS itself was retired
# 2026-08-21 (instance stopped, kept only as cold fallback) in favor of
# matcha-postgres-prod, a Postgres container on a dedicated DB EC2
# (13.56.253.173:5433). This script dumps that.
#
# Design:
#   - Reads DATABASE_URL from ~/matcha/.env.backend (already points at the
#     DB EC2) and normalizes it for libpq — the app value is asyncpg-shaped
...
echo "$(date): Starting prod logical backup (streaming to S3)..."
...
# Retention prune (S3 side, postgres-selfhosted/ prefix only). `|| true` on
# the ls: on the very first run the prefix may not exist yet and a nonzero ls
# under pipefail would turn a SUCCESSFUL backup into a failed-looking log.
(aws s3 ls "$S3_BUCKET/postgres-selfhosted/" || true) | while read -r line; do
  ...
  if [[ -n "$file_date" && -n "$file_name" ]]; then
    ...
    if [[ $file_age -gt $RETENTION_DAYS ]]; then
      ...
      aws s3 rm "$S3_BUCKET/postgres-selfhosted/$file_name" --only-show-errors
    fi
  fi
done

echo "$(date): prod backup complete"
