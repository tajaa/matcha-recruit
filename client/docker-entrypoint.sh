...
if [ -n "$API_URL" ]; then
    ...
    find /usr/share/nginx/html -type f -name "*.js" -exec sed -i "s|http://localhost:8001|$API_URL|g" {} \;
fi

if [ -n "$WS_URL" ]; then
    echo "Injecting WS_URL: $WS_URL"
    # Replace WebSocket URL
    find /usr/share/nginx/html -type f -name "*.js" -exec sed -i "s|ws://localhost:8001|$WS_URL|g" {} \;
fi
