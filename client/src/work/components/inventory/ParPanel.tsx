...
export default function ParPanel({ item, forecastEnabled, onUpdated }: {
  item: InventoryItem
  ...
  return <section className="rounded-xl border border-w-line bg-w-surface p-4">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <Button size="sm" variant={item.par_source === 'auto' ? 'secondary' : 'ghost'} disabled={saving} onClick={() => void toggleEnrollment()}>{item.par_source === 'auto' ? 'Auto-managed' : 'Enable auto par'}</Button>
    </div>
