...
type Step = { key: string; label: string; title: string; description: string; location: string; bullets: string[]; icon: LucideIcon }
...
const STEPS: Step[] = [
  ...
  { key: 'protect', label: 'Protect', title: 'Make perishability visible', description: 'On each item, add category, shelf life, and usable yield, then receive dated lots.', location: 'Inventory → select an item → Perishable settings and Lots expiring within a year', bullets: ['Shelf life caps a recommendation to stock that can be used in time.', 'Dated lots deplete earliest-expiring first as stock is used or discarded.', 'Yield improves recipe-based theoretical usage.'], icon: PackageCheck },
