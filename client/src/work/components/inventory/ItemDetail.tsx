  ...
  return (
    <div className="h-full overflow-y-auto bg-w-bg text-w-text">
      <div className="mx-auto max-w-[1200px] space-y-4 p-3 sm:p-4">
        ...
        <section className="rounded-xl border border-w-line bg-w-surface p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-medium text-w-text">Perishable settings</h2><p className="mt-1 text-xs text-w-dim">Yield adjusts theoretical recipe usage; shelf life bounds replenishment.</p></div><div className="flex gap-2"><InventoryHelpButton onClick={() => setHelp(INVENTORY_HELP.detailPerishable)} /><Button size="sm" disabled={savingPerishable} onClick={() => void savePerishableSettings()}>Save</Button></div></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><Input label="Category" value={categoryInput} onChange={(event) => setCategoryInput(event.target.value)} className="border-w-line bg-w-surface2" /><Input label="Shelf life (days)" type="number" min="1" max="3650" value={shelfLifeInput} onChange={(event) => setShelfLifeInput(event.target.value)} className="border-w-line bg-w-surface2" /><Input label="Usable yield (0–1)" type="number" min="0.01" max="1" step="0.01" value={yieldInput} onChange={(event) => setYieldInput(event.target.value)} className="border-w-line bg-w-surface2" /></div></section>

        <ParPanel item={item} forecastEnabled={canForecast} onUpdated={load} onExplain={() => setHelp(INVENTORY_HELP.detailPar)} />
