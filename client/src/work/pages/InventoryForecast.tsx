...
function ForecastRow({ line }: { line: ForecastLine }) {
  ...
  return <tr className="text-w-dim"><td className="px-4 py-3"><div className="font-medium text-w-text">{line.name}</div><div className="text-[11px] text-w-faint">{status}{line.unit ? ` · ${line.unit}` : ''}</div></td><td className="px-3 py-3">{line.history_nonzero_days} days</td><td className="px-3 py-3">{formatNumber(line.average_daily_demand)}</td><td className="px-3 py-3">{line.current_quantity === null ? 'Unknown' : formatNumber(line.current_quantity)}</td><td className="px-3 py-3">{line.runout_date ?? '—'}</td><td className="px-3 py-3">{line.order_by_date ?? '—'}</td><td className="px-4 py-3 text-right font-medium text-w-text">{line.suggested_quantity === null ? '—' : formatNumber(line.suggested_quantity)}</td></tr>
}
