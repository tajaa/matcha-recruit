...
export default function InventoryWaste() {
  ...
  async function load() {
    ...
    try {
      ...
      setSummary(nextSummary); setAtRisk(nextRisk.lines); setReasonRollup(byReason); setCategoryRollup(byCategory); setItemRollup(byItem); setLots(expiring.lots); setVariance(usage.lines); setItems(itemResult.items); setParLines(forecast?.lines ?? [])
