import { Archive, BookOpen, RotateCcw, Star } from 'lucide-react'
...
export default function JournalList({ journals, selectedId, archived, userId, onSelect, onArchive, onRestore }: Props) {
  const starred = useStars(userId, 'journals')
  return (
    <div className="max-h-48 overflow-y-auto border-r border-w-line bg-w-bg md:h-full md:max-h-none md:w-64 md:shrink-0">
      ...
      {journals.map((journal) => (
        <div key={journal.id} className={`group border-b border-w-line/50 px-3 py-2 ${selectedId === journal.id ? 'bg-w-surface2' : 'hover:bg-w-surface2/40'}`}>
          <div className="flex items-center gap-1"><button onClick={() => onSelect(journal.id)} className="min-w-0 flex-1 text-left">
          ...
          </button><button onClick={() => toggleStar(userId, 'journals', journal.id)} aria-label={`${starred.includes(journal.id) ? 'Unstar' : 'Star'} ${journal.title}`} className={starred.includes(journal.id) ? 'text-w-accent' : 'text-w-faint sm:opacity-0 sm:group-hover:opacity-100 focus:opacity-100'}><Star size={13} fill={starred.includes(journal.id) ? 'currentColor' : 'none'} /></button></div>
