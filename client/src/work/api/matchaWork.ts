...
export function agentFetchEmails() {
  return api.post<{ emails: import('../types').AgentEmail[] }>('/matcha-work/agent/email/fetch')
}
...
export function listProjects(status?: string) {
  const qs = status ? `?status=${encodeURIComponent(status)}` : ''
  return api.get<import('../types').MWProject[]>(`/matcha-work/projects${qs}`)
}
...
export function createProjectNew(
  ...
  return api.post<import('../types').MWProject>('/matcha-work/projects', {
    title,
...
export function getProjectDetail(id: string) {
  return api.get<import('../types').MWProject>(`/matcha-work/projects/${id}`)
}

export function updateProjectMeta(id: string, updates: Record<string, unknown>) {
  return api.patch<import('../types').MWProject>(`/matcha-work/projects/${id}`, updates)
}
...
export function editDiagramAI(projectId: string, sectionId: string, instruction: string, region?: { x: number; y: number; width: number; height: number }) {
  return api.post<import('../types').MWProject>(`/matcha-work/projects/${projectId}/sections/${sectionId}/edit-diagram`, { instruction, ...(region ? { region } : {}) })
}

export function editDiagramText(projectId: string, sectionId: string, edits: { old_text: string; new_text: string }[]) {
  return api.post<import('../types').MWProject>(`/matcha-work/projects/${projectId}/sections/${sectionId}/edit-diagram-text`, { edits })
}

export function saveDiagramSVG(projectId: string, sectionId: string, svg: string) {
  return api.post<import('../types').MWProject>(`/matcha-work/projects/${projectId}/sections/${sectionId}/save-diagram`, { svg })
}

export function createProjectChat(projectId: string, title?: string) {
  return api.post<import('../types').MWThread>(`/matcha-work/projects/${projectId}/chats`, { title })
}

// ── Collaborators ──

export function listCollaborators(projectId: string) {
  return api.get<import('../types').ProjectCollaborator[]>(`/matcha-work/projects/${projectId}/collaborators`)
}

export function addCollaborator(projectId: string, userId: string) {
  return api.post<import('../types').ProjectCollaborator[]>(`/matcha-work/projects/${projectId}/collaborators`, { user_id: userId })
}

export function removeCollaborator(projectId: string, userId: string) {
  return api.delete<import('../types').ProjectCollaborator[]>(`/matcha-work/projects/${projectId}/collaborators/${userId}`)
}
...
export function analyzeProjectCandidates(projectId: string) {
  return api.post<{ analyzed: number; candidates: import('../types').ResumeCandidate[] }>(
...
export function rejectProjectCandidate(
  ...
  return api.post<{ project: import('../types').MWProject; email_sent: boolean }>(
...
export function populatePostingFromChat(projectId: string, content: string) {
  return api.post<import('../types').MWProject>(
