...
export function ParticleSphere({ className = '' }: ParticleSphereProps) {
  ...
  useEffect(() => {
    ...
    // Grayscale / Zinc color variations
    const baseColor = new THREE.Color(0xffffff); // white
    const darkColor = new THREE.Color(0xa1a1aa); // zinc-400
    const lightColor = new THREE.Color(0xe4e4e7); // zinc-200
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="w-[60%] h-[60%] rounded-full bg-white/5 blur-[60px]" />
      </div>
    </div>
  );
}
