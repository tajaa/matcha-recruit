...
export function AsciiHalftone() {
  ...
  useEffect(() => {
    ...
    function getIntensity(nx: number, ny: number, t: number) {
      ...
      const angle = nx * 3 + ny * 4 + Math.sin(nx * 2 + t * 0.06) * 0.5
      v += (Math.sin(angle + t * 0.05) * 0.5 + 0.5) * 0.3

      const sCurve1 = ny - (0.5 + Math.sin(nx * Math.PI * 2 + t * 0.12) * 0.25)
      v += Math.exp(-sCurve1 * sCurve1 * 12) * 0.5

      const sCurve2 = ny - (0.3 + Math.cos(nx * Math.PI * 1.5 - t * 0.08) * 0.2)
      v += Math.exp(-sCurve2 * sCurve2 * 18) * 0.35

      const sCurve3 = ny - (0.75 + Math.sin(nx * Math.PI * 2.5 + t * 0.06) * 0.12)
