...
const linkIdle = 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900'
  ...
  return (
    <aside className="flex h-screen w-60 shrink-0 flex-col border-r border-zinc-200 bg-white">
      <div className="flex items-center gap-2 px-5 py-5">
        ...
        <span className="text-lg font-semibold tracking-tight text-zinc-900">Cappe</span>
      </div>
      ...
      <div className="border-t border-zinc-200 px-3 py-3">
        <div className="px-2 pb-2">
          <div className="truncate text-sm font-medium text-zinc-800">{account?.name || 'Account'}</div>
          <div className="truncate text-xs text-zinc-500">{account?.email}</div>
          {account?.plan && account.plan !== 'free' && (
            <span className="mt-1 inline-block rounded bg-emerald-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-emerald-700">
              {account.plan}
            </span>
          )}
        </div>
