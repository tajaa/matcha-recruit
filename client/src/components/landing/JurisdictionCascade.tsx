...
export function JurisdictionCascade() {
  ...
  return (
    <div ref={ref} className="relative h-[460px] lg:h-[520px] overflow-hidden bg-zinc-950 flex flex-col p-6" style={{ backgroundImage: SCAN_LINE_BG }}>
      ...
      {/* Timeline zone — nodes, source cards, and step labels all live here,
          centered within this zone only (not the full card). The alert
          panel below gets its own reserved zone so the two can never
          overlap, regardless of content length or breakpoint. */}
      <div className="relative flex-1 flex items-center justify-center px-4 sm:px-8 pt-10">
        ...
        <motion.div
          className="absolute left-0 h-[2px] bg-amber-500 shadow-[0_0_10px_#f59e0b]"
          initial={{ width: '0%' }}
          animate={{ width: currentStep === 0 ? '0%' : currentStep === 1 ? '16%' : currentStep === 2 ? '42%' : currentStep === 3 ? '68%' : '94%' }}
          transition={{ duration: 1.5, ease: 'easeInOut' }}
          style={{ top: 'calc(50% - 0.5px)', maxWidth: '100%' }}
        />
      ...
      {/* Alert zone — reserved height, always present, so the panel below
          never competes with the step labels in the timeline zone above. */}
      <div className="relative h-[140px] shrink-0">
        <AnimatePresence>
          {(currentStep === 3 || currentStep === 4) && (
            <motion.div
              className="absolute inset-x-0 sm:inset-x-6 bottom-0 border border-amber-500/40 bg-amber-950/60 backdrop-blur-md p-4 rounded z-30 shadow-[0_10px_30px_-10px_rgba(245,158,11,0.3)]"
