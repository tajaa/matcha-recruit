...
import { getLastRequestId, reportReactError } from '../../api/errorReporter'
...
interface State {
  hasError: boolean
  error: Error | null
  // Best-effort request ID from the last API response before the crash — a
  // reference the user can quote back to support. Snapshotted at catch time
  // since getLastRequestId() is a mutable module-level value.
  referenceId: string | null
}

class ErrorBoundaryInner extends Component<Props, State> {
  state: State = { hasError: false, error: null, referenceId: null }
  ...
  componentDidCatch(error: Error, info: ErrorInfo): void {
    ...
    this.setState({ referenceId: getLastRequestId() })
          ...
          <div className="max-w-md text-center">
            ...
            {this.state.referenceId && (
              <p className="text-xs text-zinc-500 mb-4">
                Reference: <span className="font-mono">{this.state.referenceId}</span>
              </p>
            )}
            <button
