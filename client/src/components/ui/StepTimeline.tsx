...
// Local Step type — deliberately NOT `work/types.ts`'s HuumeStep. `components/ui`
// is shared across every app in this SPA and must not import from `work/`
// (client/CLAUDE.md boundary rule); Cappe's parallel-impl precedent applies
// here too. Same shape as HuumeStep by construction (both come off a
// StepRecorder-shaped backend), so a step from either agentic loop renders
// identically without either side importing the other.
export type Step = {
  seq: number
  tool: string
...
const KIND_ICON: Record<Step['kind'], typeof FileSearch> = {
  read: FileSearch,
  staged: Clock,
  write: Bot,
  finish: CheckCircle2,
...
function statusColor(status: Step['status'], lm?: boolean) {
  switch (status) {
    case 'ok':
...
/** A per-turn agentic-loop tool-call timeline, rendered under an assistant
 * bubble from persisted message metadata or live from accumulated `step` SSE
 * frames while a turn is streaming. `headerLabel` names which loop this is
 * (e.g. "Pilot", "Huume-equivalent") since this component serves more than
 * one agentic surface. Rows carrying `args`/`result` are expandable. */
export default function StepTimeline({ steps, headerLabel = 'Agent', lightMode }: {
  steps: Step[]; headerLabel?: string; lightMode?: boolean
}) {
  ...
  return (
    <div className="mt-2.5 space-y-1 border-t pt-2" style={{ borderColor: lightMode ? '#e4e4e7' : '#3f3f46' }}>
      <span className={`text-[10px] uppercase tracking-wide ${lightMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
        {headerLabel} · {steps.length} step{steps.length !== 1 ? 's' : ''}
      </span>
