...
import type { CappePost } from '../../types'
...
export default function Blog() {
  ...
  return (
    <SurfaceShell
      ...
      actions={
        <button onClick={startNew} className="flex items-center gap-1.5 rounded-lg bg-emerald-500 px-3 py-2 text-sm font-semibold text-zinc-950 hover:bg-emerald-400">
    ...
    >
      ...
      {editing && (
        <form onSubmit={save} className="mb-6 space-y-3 rounded-2xl border border-zinc-800 bg-zinc-900 p-5 shadow-sm">
          <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Post title" className="w-full rounded-lg border border-zinc-700 bg-zinc-950 text-zinc-100 placeholder:text-zinc-500 px-3 py-2 text-sm outline-none focus:border-emerald-500" />
          <input value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} placeholder="Excerpt (optional)" className="w-full rounded-lg border border-zinc-700 bg-zinc-950 text-zinc-100 placeholder:text-zinc-500 px-3 py-2 text-sm outline-none focus:border-emerald-500" />
          <ImageUpload siteId={siteId || ''} value={form.cover_image_url} onChange={(url) => setForm({ ...form, cover_image_url: url })} placeholder="Cover image URL (optional)" />
          <textarea value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} placeholder="Write your post (Markdown)…" rows={10} className="w-full rounded-lg border border-zinc-700 bg-zinc-950 text-zinc-100 placeholder:text-zinc-500 px-3 py-2 text-sm outline-none focus:border-emerald-500" />
          <div className="flex items-center gap-3">
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="rounded-lg border border-zinc-700 bg-zinc-950 text-zinc-100 placeholder:text-zinc-500 px-2 py-2 text-sm">
            ...
            <button type="button" onClick={() => { setEditing(false); setForm(EMPTY); setEditId(null) }} className="text-sm text-zinc-500 hover:text-zinc-200">Cancel</button>
          </div>
        </form>
      )}
          ...
          {posts.map((p) => (
            <div key={p.id} className="flex items-center gap-4 px-5 py-3">
              <div className="min-w-0 flex-1">
                <div className="truncate font-medium text-zinc-100">{p.title}</div>
              ...
              <button onClick={() => startEdit(p)} className="text-zinc-400 hover:text-emerald-400"><Pencil className="h-4 w-4" /></button>
