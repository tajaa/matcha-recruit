...
export default function WerkLiteHome() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <p className="text-sm text-zinc-500 mb-8">Team chat, calls, and boards.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
            <div className="flex items-center gap-2 text-zinc-200 font-medium text-sm">
              <Hash size={16} className="text-emerald-500" /> Channels
            </div>
          ...
          <div className="p-2">
            {channels.length === 0 ? (
              <p className="px-2 py-6 text-center text-[13px] text-zinc-600">No channels yet — create one to start chatting.</p>
            ) : (
              channels.map((ch) => (
                <button
                  key={ch.id}
                  onClick={() => navigate(`${base}/channels/${ch.id}`)}
                  className="w-full flex items-center gap-2 px-2.5 py-2 rounded-md text-[13px] text-zinc-300 hover:bg-zinc-800/50 transition-colors text-left"
                >
                  ...
                  {ch.unread_count > 0 && (
                    <span className="w-4 h-4 rounded-full bg-emerald-600 text-[9px] font-bold text-white flex items-center justify-center shrink-0">
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
            ...
            {canCreateChannel && (
              <button
                ...
                className="p-1.5 rounded text-zinc-500 hover:text-emerald-400 hover:bg-zinc-800 transition-colors disabled:opacity-50"
                title="New board"
              >
          ...
          <div className="p-2">
            {boards.length === 0 ? (
              <p className="px-2 py-6 text-center text-[13px] text-zinc-600">No boards yet — create one to track work.</p>
            ) : (
              boards.map((b) => (
                <button
                  key={b.id}
