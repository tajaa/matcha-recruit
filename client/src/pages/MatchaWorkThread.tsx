...
function HandbookPreview({ state }: { state: MWDocumentState }) {
  ...
  if (state.handbook_profile) {
    for (const [key, label] of HANDBOOK_PROFILE_LABELS) {
      ...
      if (val !== undefined) profileFlags.push([label, val]);
    }
  }
  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        {/* Progress bar (before generation) */}
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {previewPanelOpen && hasPreviewContent && (
            <div className="flex md:hidden border border-white/10">
              ...
              >{(isOfferLetter || hasOfferLetterPreviewContent) ? 'Preview' : (isWorkbook || hasWorkbookPreviewContent) ? 'Workbook' : (isPresentation || hasPresentationPreviewContent) ? 'Presentation' : (isHandbook || hasHandbookPreviewContent) ? 'Handbook' : hasOnboardingPreviewContent ? 'Onboarding' : 'Summary'}</button>
            </div>
          )}
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-8">
                ...
                <div className="mt-3 text-[11px] text-zinc-500 max-w-sm">
                  Default mode: US HR chat. Skills: offer letters, anonymized reviews, HR workbooks, employee onboarding, employee handbooks. Ask naturally and Matcha will route commands when supported.
                </div>
