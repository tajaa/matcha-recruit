...
// ─── Offer Lifecycle Wizard ──────────────────────────────────────────────────

type OfferStepIcon = 'draft' | 'generate' | 'send' | 'track' | 'onboard';
...
const OFFER_CYCLE_STEPS: OfferWizardStep[] = [
  {
    id: 1,
    icon: 'draft',
    title: 'Draft Package',
...
function OfferCycleWizard({ offerLetters }: { offerLetters: OfferLetter[] }) {
  const storageKey = 'offer-wizard-collapsed-v1';
  ...
  const activeStep = offerLetters.some(l => l.status === 'accepted') ? 5 
    ...
    <div className="border border-white/10 bg-zinc-950/60 mb-10">
      ...
      >
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Offer Lifecycle</span>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-600 hidden sm:inline">
              {OFFER_CYCLE_STEPS[activeStep - 1].title}
            </span>
          </div>
        </div>
        ...
        <div className="border-t border-white/10">
          <div className="relative px-5 pt-5 pb-2 overflow-x-auto no-scrollbar">
            <div className="flex items-start gap-0 min-w-max">
              {OFFER_CYCLE_STEPS.map((step, idx) => {
                  ...
                  <div key={step.id} className="flex items-start">
                    <div className="flex flex-col items-center w-28">
                      <div className={`relative w-9 h-9 rounded-full border-2 flex items-center justify-center text-sm transition-all ${
                        ...
                        {isComplete ? '✓' : <OfferCycleIcon icon={step.icon} className="w-4 h-4" />}
                      </div>
                    ...
                    {idx < OFFER_CYCLE_STEPS.length - 1 && (
          ...
          <div className="mx-5 mb-5 p-4 bg-white/[0.03] border border-white/10">
            <div className="flex items-start gap-3">
              <span className="text-xl flex-shrink-0 text-zinc-200">
                <OfferCycleIcon icon={OFFER_CYCLE_STEPS[activeStep - 1].icon} className="w-5 h-5" />
              </span>
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-zinc-400 leading-relaxed mb-2">
                  {OFFER_CYCLE_STEPS[activeStep - 1].description}
                </p>
                {OFFER_CYCLE_STEPS[activeStep - 1].action && (
