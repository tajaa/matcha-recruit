...
export function RegisterBrokerClient() {
  ...
  const [invite, setInvite] = useState<BrokerClientInviteDetails | null>(null);
  ...
  useEffect(() => {
    ...
    brokerClientInviteApi.validate(token)
      .then((data) => {
        setInvite(data);
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await brokerClientInviteApi.accept(token!, {
        password,
        name: name || undefined,
        phone: phone || undefined,
        job_title: jobTitle || undefined,
      });
      // Tokens are set by the API call; load the full user profile
      try {
        await refreshUser();
      } catch (err) {
        console.warn('Failed to load profile after broker client registration:', err);
      }
      navigate('/app');
    } catch (err) {
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-[420px]">
        <div className="mb-4 bg-emerald-50 border border-emerald-200 rounded-sm p-3 flex items-center gap-2">
          ...
          <span className="text-xs text-emerald-800">
            <strong>{invite.broker_name}</strong> has set up <strong>{invite.company_name}</strong> for you. Create your password to get started.
          </span>
        </div>
...
export default RegisterBrokerClient;
