        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <div className="flex flex-col gap-5">
              <div>
                <label className="block text-xs mb-2" style={{ color: MUTED }}>Total headcount</label>
                <input
                  type="number"
                  min={0}
                  value={headcount}
              ...
              <div>
                <label className="block text-xs mb-2" style={{ color: MUTED }}>Voluntary leavers per year</label>
                <input
                  type="number"
                  min={0}
                  value={annualLeavers}
              ...
              <div>
                <label className="block text-xs mb-2" style={{ color: MUTED }}>Average salary</label>
                <input
                  type="number"
                  min={0}
                  step={1000}
              ...
              <div>
                <label className="block text-xs mb-2" style={{ color: MUTED }}>Role tier (replacement cost multiplier)</label>
                <select
                  value={tier}
          ...
          <section className="flex flex-col gap-4">
            <ResultBox label="Annual turnover cost" value={fmtUSD(result.annualCost)} sub={`${fmtUSD(result.monthlyCost)}/month`} large />
            <div className="grid grid-cols-2 gap-4">
              <ResultBox label="Per-leaver cost" value={fmtUSD(result.replacementCostPerLeaver)} sub={`${(result.multiplier * 100).toFixed(0)}% of salary`} />
              <ResultBox label="Turnover rate" value={fmtPct(result.turnoverRate)} sub="leavers ÷ headcount" />
            </div>
            <div
...
function ResultBox({ label, value, sub, large }: { label: string; value: string; sub?: string; large?: boolean }) {
  return (
    <div
