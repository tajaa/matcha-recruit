    ...
    <div style={embedded ? { color: t.ink } : { backgroundColor: t.bg, color: t.ink, minHeight: '100vh' }}>
      ...
      <main className={embedded ? '' : 'pt-28 pb-20 max-w-[1100px] mx-auto px-6 sm:px-10'}>
        <nav className={`flex items-center gap-2 text-xs mb-8 ${embedded ? 'text-vsc-text/40' : ''}`} style={embedded ? undefined : { color: t.muted }}>
          ...
          <span className={embedded ? 'text-vsc-text/60' : ''} style={embedded ? undefined : { color: t.ink }}>Glossary</span>
        </nav>
        ...
        <div className="mb-10">
          <div
            className={embedded
              ? 'flex items-center gap-3 px-4 h-10 rounded-lg border border-vsc-border bg-vsc-bg max-w-md'
              : 'flex items-center gap-3 px-4 h-12 rounded-full max-w-md'}
            style={embedded ? undefined : { border: `1px solid ${t.line}` }}
          >
            <Search className={`w-4 h-4 ${embedded ? 'text-vsc-text/40' : ''}`} style={embedded ? undefined : { color: t.muted }} />
            <input
              type="text"
              placeholder="Search terms (FLSA, COBRA, retaliation…)"
              value={query}
              onChange={e => setQuery(e.target.value)}
              className={`flex-1 bg-transparent outline-none text-sm ${embedded ? 'text-vsc-text' : ''}`}
          ...
          {ALPHA.map(letter => {
            ...
            return has ? (
              <a
                key={letter}
                href={`#letter-${letter}`}
                className={`w-8 h-8 inline-flex items-center justify-center text-sm rounded ${embedded ? 'border border-vsc-border text-vsc-text hover:border-vsc-text/40 transition-colors' : 'transition-opacity hover:opacity-60'}`}
                style={embedded ? undefined : { color: t.ink, border: `1px solid ${t.line}` }}
              >
                {letter}
              </a>
            ) : (
              <span
                key={letter}
                className={`w-8 h-8 inline-flex items-center justify-center text-sm rounded ${embedded ? 'text-vsc-text/20' : ''}`}
                style={embedded ? undefined : { color: 'rgba(0,0,0,0.2)' }}
              >
                {letter}
              </span>
            )
        ...
        {presentLetters.length === 0 ? (
          <p className={embedded ? 'text-vsc-text/50' : ''} style={embedded ? undefined : { color: t.muted }}>No terms match "{query}".</p>
        ) : (
          presentLetters.map(letter => (
            <section key={letter} id={`letter-${letter}`} className="mb-12 scroll-mt-28">
              <h2
                className={embedded ? 'text-xl font-semibold text-vsc-text mb-4' : 'text-3xl mb-6'}
              ...
              <div className="flex flex-col gap-3">
                {grouped[letter].map(term => (
                  <Link
                    ...
                    className={embedded
                      ? 'border border-vsc-border bg-vsc-panel rounded-xl p-5 block hover:border-vsc-text/30 transition-colors'
                      : 'block p-5 rounded-xl transition-opacity hover:opacity-80'}
                    style={embedded ? undefined : { border: `1px solid ${t.line}` }}
                  >
                    <div className="flex items-baseline gap-3 mb-1 flex-wrap">
                      <h3
                        className={embedded ? 'text-base font-semibold text-vsc-text' : 'text-lg'}
                        style={embedded ? undefined : { fontFamily: t.display, color: t.ink, fontWeight: 500 }}
                      >
                        {term.abbreviation ?? term.term}
                      </h3>
                      {term.abbreviation && (
                        <span className={embedded ? 'text-sm text-vsc-text/50' : 'text-sm'} style={embedded ? undefined : { color: t.muted }}>
                          {term.term}
                        </span>
                      )}
                      <span
                        className={`text-[10px] tracking-wider px-2 py-0.5 rounded ml-auto ${embedded ? 'border border-vsc-border text-vsc-text/40' : ''}`}
        ...
        {!embedded && (
          <section className="mt-14 max-w-xl mx-auto">
            <NewsletterSignup
              source="glossary"
              variant="card"
              headline="HR terms, plain English, weekly."
              description="One short brief — definitions, employment-law updates, the occasional template."
            />
          </section>
        )}
      </main>
