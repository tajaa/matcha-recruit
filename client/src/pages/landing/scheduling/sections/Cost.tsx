          ...
          <Reveal className="flex flex-col py-12 lg:border-r lg:pr-14" style={{ borderColor: RULE }}>
            ...
            <dl className="mt-auto grid grid-cols-3 items-end gap-6 pt-12">
              {[
                { k: 'Of sales', v: `${pct}%` },
                { k: 'Overtime', v: '$0' },
                { k: 'Unpriced', v: '12h', color: BOARD.AMBER },
              ].map((x) => (
                <div key={x.k}>
                  <dt style={mono('10px', { color: hexA(PAPER, 0.55) })}>{x.k}</dt>
                  <dd className="mt-2 text-[1.9rem] font-normal leading-none tracking-[-0.04em]" style={{ color: x.color ?? PAPER, fontVariantNumeric: 'tabular-nums' }}>
                    {x.v}
                  </dd>
                </div>
              ))}
            ...
            <table className="mt-8 w-full border-collapse" style={mono('11px', { color: PAPER })}>
              ...
              <tbody>
                ...
                <tr style={{ borderTop: `1px solid ${RULE}`, color: hexA(PAPER, 0.55) }}>
                  <td className="py-3.5">
                    <span className="mr-3 inline-block h-1.5 w-4 rounded-full align-middle" style={{ boxShadow: `inset 0 0 0 1px ${BOARD.AMBER}` }} />
                    Prep · new hire
                  </td>
                  <td className="py-3.5 text-right">1</td>
                  <td className="py-3.5 text-right">12h</td>
                  <td className="py-3.5 text-right" style={{ color: BOARD.AMBER }}>
                    Unpriced
                  </td>
                </tr>
              </tbody>
