...
export default function ENPS() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <LifecycleWizard
        steps={ENPS_CYCLE_STEPS}
        activeStep={surveys.some(s => (s.status === 'closed' || s.status === 'archived')) ? 5
                  : activeSurveys.length > 0 ? 3
                  : surveys.some(s => s.status === 'draft') ? 2
                  : 1}
        title="eNPS Cycle"
        storageKey="enps-wizard-collapsed-v1"
      />
