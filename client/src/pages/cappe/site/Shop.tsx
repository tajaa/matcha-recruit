...
export default function Shop() {
  ...
  return (
    <SurfaceShell title="Shop" subtitle="Products you sell on this site.">
      ...
      <form onSubmit={addProduct} className="mb-6 grid grid-cols-1 gap-3 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:grid-cols-12">
        <input
          value={form.name}
          onChange={(e) => setForm({ ...form, name: e.target.value })}
          placeholder="Product name"
          className="rounded-lg border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-emerald-500 sm:col-span-4"
        />
        <input
          value={form.price}
          ...
          step="0.01"
          min="0"
          className="rounded-lg border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-emerald-500 sm:col-span-2"
        />
        <input
        ...
        <input
          ...
          className="rounded-lg border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-emerald-500 sm:col-span-3"
        />
        <button
          type="submit"
          disabled={adding}
          className="flex items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60 sm:col-span-1"
        >
        ...
        <div className="divide-y divide-zinc-100 rounded-2xl border border-zinc-200 bg-white">
          {products.map((p) => (
            <div key={p.id} className="flex items-center gap-4 px-5 py-3">
              ...
              <button onClick={() => remove(p.id)} className="text-zinc-400 hover:text-red-600">
