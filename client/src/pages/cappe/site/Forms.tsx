...
export default function Forms() {
  ...
  return (
    <SurfaceShell
      ...
      actions={
        <button onClick={() => setCreating((v) => !v)} className="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700">
    ...
    >
      ...
      {creating && (
        <form onSubmit={create} className="mb-6 space-y-3 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
          <input
            value={name}
            ...
            {fields.map((f, i) => (
              <div key={i} className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-zinc-300 px-3 py-1.5 text-sm outline-none focus:border-emerald-500"
                />
                <select value={f.type} onChange={(e) => setField(i, { type: e.target.value })} className="rounded-lg border border-zinc-300 px-2 py-1.5 text-sm">
                  {FIELD_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
                <label className="flex items-center gap-1 text-xs text-zinc-500">
                  <input type="checkbox" checked={f.required} onChange={(e) => setField(i, { required: e.target.checked })} /> req
                </label>
                <button type="button" onClick={() => setFields((fs) => fs.filter((_, idx) => idx !== i))} className="text-zinc-400 hover:text-red-600">
                  <X className="h-4 w-4" />
                </button>
              </div>
            ))}
            <button type="button" onClick={() => setFields((f) => [...f, { key: '', label: '', type: 'text', required: false }])} className="text-xs font-medium text-emerald-700 hover:underline">
              + Add field
            </button>
          </div>
      ...
      {forms === null ? (
        ...
        <div className="space-y-3">
          {forms.map((form) => (
            <div key={form.id} className="rounded-2xl border border-zinc-200 bg-white">
              <div className="flex items-center gap-3 px-5 py-3">
                <button onClick={() => toggle(form)} className="text-zinc-400 hover:text-zinc-700">
                  {openId === form.id ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </button>
                <div className="min-w-0 flex-1">
                  <div className="font-medium text-zinc-900">{form.name}</div>
                  <div className="text-xs text-zinc-400">/{form.slug} · {form.fields.length} field(s)</div>
                </div>
                <button onClick={() => removeForm(form.id)} className="text-zinc-400 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
              </div>
              {openId === form.id && (
                <div className="border-t border-zinc-100 bg-zinc-50 px-5 py-3">
                  {!subs[form.id] ? (
                    ...
                    <ul className="space-y-2">
                      {subs[form.id].map((s) => (
                        <li key={s.id} className="rounded-lg bg-white p-3 text-sm shadow-sm">
                          <div className="mb-1 text-xs text-zinc-400">{new Date(s.created_at).toLocaleString()}</div>
                          {Object.entries(s.data).map(([k, v]) => (
                            <div key={k} className="text-zinc-700"><span className="font-medium">{k}:</span> {String(v)}</div>
                          ))}
                        </li>
                      ))}
                    </ul>
