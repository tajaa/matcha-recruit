      ...
      <div className="flex h-full min-h-0 min-w-0 flex-col bg-zinc-950 text-zinc-100">
        ...
        {weekRules && !weekRules.established && locationId && bodyMode === 'grid' && (
          <div className="flex flex-wrap items-center gap-3 border-b border-amber-500/25 bg-amber-500/[0.08] px-4 py-2 text-xs text-amber-100 md:px-6">
            ...
            <span>
              {currentLocationName || 'This location'} is still missing{' '}
              {weekRules.missing.map((field) => WEEK_RULE_LABELS[field]).join(', ')}. Huume can’t build a week until these are saved.
            </span>
