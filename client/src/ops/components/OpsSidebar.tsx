...
import { CalendarDays, Hash, Home, Link2, Package, PanelLeftClose, ClipboardList, ShieldCheck } from 'lucide-react'
...
export default function OpsSidebar({ open, onToggle }: Props) {
  ...
  const canAccessFeature = (feature: string) => me?.user.role === 'admin' || hasFeature(feature)
  // Sym-links live on the /app page (not an Ops route) and its API admits company
  // admins only — `employee` members of an Ops channel must not see the row.
  const canSendSymlinks = me?.user.role === 'admin' || (me?.user.role === 'client' && hasFeature('symlink'))
  ...
  if (!open) return <aside className="w-12 shrink-0 border-r border-w-line bg-w-surface py-2"><button onClick={onToggle} className="mx-auto block p-2 text-w-dim hover:text-white" title="Open sidebar"><PanelLeftClose size={16} className="rotate-180" /></button><button onClick={() => go('')} className="mx-auto mt-2 block p-2 text-w-dim hover:text-white" title="Home"><Home size={16} /></button>{canAccessFeature('inventory_waste') && <button onClick={() => go('/inventory/waste')} className="mx-auto block p-2 text-w-dim hover:text-white" title="Waste & par"><Package size={16} /></button>}{canSendSymlinks && <button onClick={() => navigate('/app/symlink')} className="mx-auto block p-2 text-w-dim hover:text-white" title="Sym-links"><Link2 size={16} /></button>}<button onClick={() => go('/channels')} className="mx-auto block p-2 text-w-dim hover:text-white" title="Channels"><Hash size={16} /></button></aside>
  ...
  return <aside className="flex w-56 shrink-0 flex-col overflow-hidden border-r border-w-line bg-w-surface">
    ...
    <nav className="flex-1 space-y-1 overflow-y-auto px-2 pb-3">
      ...
      {canAccessFeature('ems') && <button onClick={() => go('/events')} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[13px] ${active(`${base}/events`) ? 'bg-w-surface2 text-white' : 'text-w-dim hover:bg-w-surface2/50 hover:text-w-text'}`}><ClipboardList size={14} /> Events</button>}
      {canAccessFeature('inventory') && <button onClick={() => go('/inventory')} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[13px] ${active(`${base}/inventory`) ? 'bg-w-surface2 text-white' : 'text-w-dim hover:bg-w-surface2/50 hover:text-w-text'}`}><Package size={14} /> Inventory</button>}
      {canAccessFeature('inventory_waste') && <button onClick={() => go('/inventory/waste')} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[13px] ${active(`${base}/inventory/waste`) ? 'bg-w-surface2 text-white' : 'text-w-dim hover:bg-w-surface2/50 hover:text-w-text'}`}><Package size={14} /> Waste & par</button>}
       {canAccessFeature('employee_schedule') && <button onClick={() => go('/schedule')} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[13px] ${active(`${base}/schedule`) ? 'bg-w-surface2 text-white' : 'text-w-dim hover:bg-w-surface2/50 hover:text-w-text'}`}><CalendarDays size={14} /> Schedule</button>}
       {canSendSymlinks && <button onClick={() => navigate('/app/symlink')} className="flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[13px] text-w-dim hover:bg-w-surface2/50 hover:text-w-text"><Link2 size={14} /> Sym-links</button>}
