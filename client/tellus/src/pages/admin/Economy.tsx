...
function EarningRules() {
  ...
  useEffect(() => { tellusApi.get<AdminEarningRule[]>('/admin/earning-rules').then(setRules) }, [])
...
function Badges() {
  ...
  useEffect(() => { tellusApi.get<AdminBadge[]>('/admin/badges').then(setBadges) }, [])
        ...
        {badges.map((b) => {
          ...
          return (
            <div key={b.key} className="flex flex-wrap items-end gap-2 border-b border-tu-border/50 pb-2 last:border-b-0">
              ...
              <div className="w-24">
                <Input
                  label="Threshold" type="number" value={d.threshold ?? b.criteria.threshold ?? ''}
                  onChange={(e) => setDrafts((ds) => ({ ...ds, [b.key]: { ...ds[b.key], threshold: Number(e.target.value) } }))}
                />
              </div>
