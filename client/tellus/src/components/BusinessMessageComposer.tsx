...
export function BusinessMessageComposer({ slug, stores }: { slug: string; stores: MessagingStore[] }) {
  ...
  async function send() {
    ...
    try {
      const result = await tellusApi.post<CommsStartResponse>(`/comms/brands/${slug}/threads`, {
        store_id: storeId || null, topic, body: body.trim(), client_message_id: crypto.randomUUID(),
      })
  ...
  return (
    <div className="mx-auto mt-4 max-w-lg rounded-xl border border-tu-border bg-tu-panel p-4 text-left">
      ...
      <p className="mt-1 text-xs text-tu-faint">A team member will reply here. You’ll need to log in to start a conversation.</p>
      <div className="mt-3 space-y-2">
        ...
        <textarea value={body} onChange={e => setBody(e.target.value)} maxLength={4000} rows={3} placeholder="What would you like to know?" className="w-full rounded-lg border border-tu-border bg-tu-panel2 px-2.5 py-2 text-sm placeholder:text-tu-faint focus:border-tu-accent focus:outline-none" />
