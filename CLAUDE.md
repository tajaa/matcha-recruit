...
- **Kanban autopr** (`.github/workflows/kanban-autopr.yml` + `scripts/kanban-autopr/`) — self-hosted-runner bot that picks a kanban card assigned to a fixed service account across four fixed Espresso projects (`todo`/`changes_requested`) and opens a draft PR — or, for a `research` card, attaches a Luna-written report (web search + repo clone + screenshots) to the card and moves it to Review; a `post-checkout` hook and a `pull_request` webhook sync the card back. Kind registry in `scripts/kanban-autopr/lib.sh` (`autopr_kind_field`). Reading the live web, staging email a human approves per-send, and driving a browser are per-board grants (`platform_settings.autopr_board_capabilities`, Admin → Settings), all default off. → full spec: `docs/ops/KANBAN_AUTOPR.md`
