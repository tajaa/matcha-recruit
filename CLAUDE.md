...
| `risk_pilot` | ❌ | **Risk Pilot builder** (full Matcha / Pro). A company-facing, **bring-your-own-data** volatility & risk analysis engine: the business opens a **session** and uploads datasets (**CSV / XLSX / financial-document PDF** — 10-Ks, P&Ls, balance sheets, loss runs, inventory). Three-stage integrity: (1) **extraction** — Gemini pulls line-items×periods from documents, which the user **confirms/edits** before analysis (`services/risk_pilot.py:extract_dataset`); (2) **computation** — a **deterministic**, stdlib-only engine (`services/risk_analyzers/`) computes the metrics via a **pluggable analyzer-pack registry** — `volatility` (σ/annualized vol, VaR95/99, CVaR, max drawdown, Sharpe-like, downside deviation, correlation matrix), `financial_ratios` (liquidity/leverage/profitability + YoY growth & trend σ), `insurance_loss` (loss ratio, frequency, severity, development), `inventory_ops` (turnover, days-on-hand, stockout, HHI) — over one **normalized** model (CSV/XLSX/doc all flatten to named series + periods + roles; row-oriented P&Ls are auto-transposed; a **semantic role map** — heuristic + user-editable — is how packs gate via `applies()`); (3) **narration** — a **grounded** AI narrates over the COMPUTED numbers and may cite ONLY real record ids (`metric:`/`ratio:`/`corr:`/`series:`/`figure:`/`compare:`), enforced by the shared `legal_defense.validate_citations`. Saves cross-dataset **comparisons** (`compare.py` — deltas/%/CAGR) and exports an analyst **report PDF** with inline pure-Python **SVG** charts (WeasyPrint spine; deterministic tiles/tables/charts rendered from stored metrics, never model text). Adds one dep (`openpyxl`, XLSX; no numpy/pandas). Tables `risk_pilot_sessions` / `_datasets` / `_comparisons` / `_messages` / `_packets` / `_audit_log` (migration `riskpilot01`). Gates `/risk-pilot/*` + the `/app/risk-pilot` page. Default off; admin-toggle; NOT bundled (paid analysis asset, like `legal_defense`). |
