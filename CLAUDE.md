...
| `inventory` | ❌ | Channel-driven inventory tracking via `@huume` — auto-created items, append-only movement ledger, internal order queue (queued→ordered→received) with in-channel confirm; full `/work` Inventory page under the **"Ops"** sidebar group. Items are per-store (`inventory_items.location_id`) when created from a location-scoped channel; a channel with no store binding shares the legacy company-wide catalog. NOT bundled. → `server/app/matcha/services/inventory/CLAUDE.md` |
...
Scheduling model: no celery-beat. Worker container runs continuously (`restart: unless-stopped`); an hourly host cron (`docker restart matcha-worker`) re-fires `@worker_ready` in `app/workers/celery_app.py`, which re-dispatches periodic tasks. Each is gated by a `scheduler_settings` row, default disabled.
