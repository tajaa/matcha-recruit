...
| `huume` | ❌ | Huume — bounded OpenAI Luna tool-calling agent in matcha-work threads; everything staged/confirm-first. Skills: onboarding, HR-ops, pilot bridges, incident-discipline. Needs `matcha_work`. → `server/app/matcha/services/huume/CLAUDE.md` |
...
| `labor_cost` | ❌ | Scheduled labor cost — the dollars behind the hours every scheduling surface already shows. Prices `employees.pay_rate` against each shift's worked minutes, overtime-aware off the **cited** thresholds and multipliers in `schedule_compliance._SCHEDULING_RULES` (FLSA § 207(a) 40h/1.5×; Cal. Lab. Code § 510 8h/12h/1.5×/2.0×), no pyramiding. Open seats price from a new `schedule_jobs.default_hourly_rate`. Missing pay data is reported **unpriced, never $0**. Wage data: endpoints drop the `individual` role (`is_labor_cost_visible`) and pilots get aggregates only. As-scheduled, never as-worked — there is no time-clock data. Requires `employee_schedule`; default off; admin-toggle; NOT bundled. → `server/app/matcha/services/scheduling/CLAUDE.md` |
| `schedule_autopilot` | ❌ | Premium forecast-driven week generation from store setup, committed sales, Google Weather, published schedule history, and the qualified roster. Deterministic and review-only: it freezes generated demand on the proposal and still uses the ordinary planner/compliance/cost/Huume approval/apply path. Requires `employee_schedule` + `huume` + `matcha_work`; default off; admin-toggle; NOT bundled. → `server/app/matcha/services/scheduling/CLAUDE.md` |
...
- **Kanban autopr** (`.github/workflows/kanban-autopr.yml` + `apps/msandbox/harness/`) — self-hosted-runner bot that picks a kanban card assigned to a fixed service account across four fixed Espresso projects (`todo`/`changes_requested`) and opens a draft PR — or, for a `research` card, attaches a Luna-written report (web search + repo clone + screenshots) to the card and moves it to Review; an `email` card instead reads the `email-<message id>.md` snapshots Espresso's Email viewer attached ("Send to board") and attaches a triage report whose reply drafts are staged for per-send approval; a `post-checkout` hook and a `pull_request` webhook sync the card back. Kind registry in `apps/msandbox/harness/lib.sh` (`autopr_kind_field`). Reading the live web, staging email a human approves per-send, driving a browser, and running email cards are per-board grants (`platform_settings.autopr_board_capabilities`, Admin → Settings), all default off. → full spec: `apps/msandbox/docs/KANBAN_AUTOPR.md`
...
- **MCP connector** (`core/services/mcp_oauth.py` + `matcha/routes/mcp_connector/` + `routes/matcha_work/connectors.py`, migration `mcpconn01`) — Matcha as a remote MCP connector for Claude / ChatGPT / Claude Code: kanban **research** cards run on the person's *own* plan (read card → research → `attach_research_report` lands the same `research-report-*.md` + Review move AutoPR does). Matcha is the OAuth 2.1 AS (MCP SDK handlers, DCR, PKCE, audience-bound opaque tokens, refresh rotation); it **never** holds a Claude/OpenAI credential — Anthropic's consumer terms forbid using a Pro/Max login in another product, so no spawning `claude -p`. AutoPR unchanged. → `docs/ops/MCP_CONNECTOR.md`
...
**⚠️ The CloudFront origin gate is load-bearing for more than origin secrecy — do not disable it casually.** `/etc/nginx/snippets/{matcha,cappe}-cloudfront-origin-gate.conf` 403s any request that arrives without the matching `X-Matcha-Origin-Verify` / `X-Cappe-Origin-Verify` header, so direct-to-origin scanners never reach a real route. Because of that, the host's fail2ban `nginx-404` jail was **disabled on 2026-09-10**: behind CloudFront it bans the *connecting* address, which is always a CloudFront edge IP, and banning one blackholes a whole POP for every real user routed through it (that is exactly what caused a 13-minute partial 504 outage that day; 140 of the 155 addresses it had ever banned were CloudFront edges, and every non-CloudFront ban it ever made predates the gate going in on 2026-08-20/21).

What that trade costs us, stated plainly so it is not rediscovered the hard way: **we lose protection against direct-to-origin 404 scanners. That is real, but currently worth zero, because the gate already 403s them. It only regains value if the gate is ever turned off.** So if you disable, bypass, or fail to reinstall that gate — on either server block — you must replace the protection in the same change: re-enable the `nginx-404` jail *with* the CloudFront ranges in `ignoreip`, or cover it at the WAF. Full incident writeup, rule inventory, and the apply/rollback commands: `docs/ops/MATCHA_EDGE.md`.
...
**`build-and-push.sh` with no target flag builds only what changed** (2026-09-07): it reads the git SHA tagged on each ECR `:latest`, diffs `server/` (minus tests/agent) and `client/` against it — uncommitted edits count — and skips the untouched image; nothing changed ⇒ exits 0 with no build. Same logic picks the `--remote` workflow target (`backend`/`frontend`/`matcha`). `--full` forces both; `--backend-only`/`--frontend-only`/`--all` are explicit as before. It prints the matching `update-ec2.sh` flag at the end.

**`update-ec2.sh` refuses a backend swap while prod is missing migrations** (2026-09-07): before touching prod it reads `alembic_version` through the live backend container (`scripts/ops-health/prod-query.sh alembic`) and diffs against `server/alembic/versions` with the stdlib `scripts/alembic_graph.py`. Interactive: offers to run `migrate-prod.sh` right there. CI / non-TTY: fails with the pending list in the step summary. `--allow-pending-migrations` (workflow input `allow_pending_migrations`) is the explicit override; a prod revision unknown to the checkout also blocks. Frontend-only deploys skip the check.
...
**Dev test account — PRIMARY (matcha-ops, restaurant/retail)**: `tessu2022+pocoffee@gmail.com` / `devpass123` in the local dev DB — client role, company **"Po Coffee Co"** (`c4c256c3-60ef-4cf5-8d4c-55e963c58416`, `signup_source=product:matcha-ops`), with `employee_schedule`/`huume`/`matcha_work`/`matcha_ops`/`inventory`/`sales_intake`/`ems`/`schedule_intelligence` enabled. Two stores: "Po Coffee Co — Downtown" (`c0ffeeee-0001-4001-8001-000000000001`) and "— Mission" (`…0002`), 15 employees, 5 jobs, 72 shifts. **Use this one by default** — restaurants/retail is the segment we actually sell into, so its data shape (per-store jobs, hourly crew, partially-filled qualified lists) is the realistic one.
⚠️ That address is a REAL deliverable Gmail alias, not a reserved test domain. Never point bulk-invite, newsletter, or any fan-out send at it — see the Test Data rules below.

**Dev test account (full-featured Matcha-X, dental)**: `maria.chen@example.com` / `devpass123` — client role, company "Sunset Smile Dental Group" (`signup_source=matcha_x`), `employee_schedule`/`huume`/`matcha_work`/`matcha_ops` enabled. Locations include "Sunset Smile Dental — Downtown" (`e628e73e-1ee3-4b29-ba6d-a7e4cbc5e895`). Secondary — reach for it when you specifically want the Matcha-X tier or a professional-services shape; verified end-to-end against `/ops/schedule/editor` on 2026-08-26.

**Alternative**: `./scripts/dev.sh` — references a discontinued sister product; do not use.

**Agent sandbox**: `msandbox` (or `./apps/msandbox/bin/agent-sandbox.sh`) runs Codex/Claude Code/OpenCode in an isolated
...
Full writeup: `apps/msandbox/docs/AGENT_SANDBOX.md`.
...
| `apps/msandbox/CLAUDE.md` | `apps/msandbox/**` — the agent sandbox, `msandbox` session manager, and the AutoPR harness (operator tooling; not part of the product) |
