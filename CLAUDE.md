...
**Or dispatch from the Actions tab / GitHub mobile app** (`.github/workflows/deploy.yml`, `workflow_dispatch`): pick target (`matcha`/`backend`/`frontend`) + optional hotfix toggle — same `update-ec2.sh` underneath. Actions releases require `main`, check migrations before the image push, and pass an ECR digest through the backend worker and blue/green swap. Build runs on the free ARM runner via OIDC; deploy job SSHs in with the same key as the laptop path; all third-party actions SHA-pinned. Detail: `docs/ops/DEPLOY.md`
