...
- **Schema drift:** `operational-integrity-checks.yml` compares exact multi-head `alembic_version` sets from the shared local dev container and live prod; on mismatch it generates a read-only, normalized schema-only dump diff. It runs on the self-hosted Mac and never starts dev Postgres. Detail: `docs/ops/DB_WORKFLOW.md`
