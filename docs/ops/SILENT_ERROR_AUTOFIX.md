...
2. Redacts credentials, emails, IPs, query-string secrets, and UUIDs before model access.
...
- Agent receives only redacted evidence.
- Agent cannot change workflows, deploy code, scripts, dependencies, migrations, or env files.
- Workflow refuses such changed paths before committing.
