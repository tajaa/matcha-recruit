...
- **Data, prod → dev:** `./scripts/refresh-dev-from-prod.sh` — **anonymized** clone of the live EC2 Postgres into dev. `pg_dump` runs in a PG15 container on the app EC2 and streams to a local staging file. `--dry-run` restores into a staging DB without swapping. After a scrubbed run, **every dev user's password becomes `devpass123`**; PII is scrubbed by `scripts/sql/anonymize_dev.sql` — Matcha tables in blocks 1–7, **Cappe (gummfit.com) consumer data in block 8** (`cappe_accounts`, subscribers, orders, bookings, clients, threads, form submissions; tokens regenerated, Stripe ids faked). The refresh script's leak gate checks both identity models; a Cappe table that gains a new email/token column must be added to block 8 *and* to that gate.
